...
class MujocoRobot(ABC):
    def __init__(
        ...
        self.f_sensor_thresh = np.array([15, 15, 15])  # 0601将20牛改为10牛
        # self.t_sensor_thresh = np.array([ft_rxyz, ft_rxyz, ft_rxyz])
        self.t_sensor_thresh = np.array([0.5, 0.5, 0.5])  # 0601将2牛米改为0.15
...
class RobotTaskEnv(gym_robotics.GoalEnv):
    ...
    def success_check(self):
        ...
        #s_check = np.array(d_bottom < 0.002, dtype=np.float64)
        return s_check
