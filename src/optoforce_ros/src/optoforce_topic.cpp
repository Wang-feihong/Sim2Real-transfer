...
int optoforce_topic::run()
{
  ...
  // //从rosparam寻找robot_description并构造一个供我们使用的RobotModel
  // robot_model_loader::RobotModelLoader robot_model_loader("robot_description");
  // robot_model::RobotModelPtr kinematic_model = robot_model_loader.getModel();

  // //使用RobotModel构造一个RobotState来维护机器人的配置。
  // //将所有处于状态的关节设置为默认值得到一个JointModelGroup，它代表一个特定组的机器人模型，例如tx90。
  // robot_state::RobotStatePtr kinematic_state(new robot_state::RobotState(kinematic_model));
  // const robot_state::JointModelGroup* joint_model_group = kinematic_model->getJointModelGroup("tx90_arm");
  
  // //计算正运动学。注意，我们想要找到熊猫连杆8的姿态，它是机器人熊猫臂群中最远端的连杆。
  // // const Eigen::Isometry3d& end_effector_state = kinematic_state->getGlobalLinkTransform("joint_6");
